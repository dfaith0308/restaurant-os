...
# 【2차 심화 2】 2026-09-09 — 참조 무결성 전수 검사

> 이 문서는 지금까지 「연결 컬럼이 **채워졌는가**」만 봤다. 채워진 값이 **실제로 존재하는 행을 가리키는가**는 확인하지 않았다.
> 그 구멍을 메운다. 이 구분이 중요한 이유는 §16에 적었다.

## 15. FK 144개 전수 검사 — **고아 참조 0건**

`public` 스키마의 단일 컬럼 외래키 **144개 전부**에 대해, 자식 행의 FK 값이 부모 테이블에 실재하는지 검사했다.

```sql
-- 144개 각각에 대해 (자동 생성 후 배치 실행)
select count(*) from <자식> t
 where t.<fk> is not null
   and not exists (select 1 from <부모> p where p.<pk> = t.<fk>)
```
...
**→ FK로 선언된 관계는 전부 온전하다.** 주문↔라인, 주문↔고객, 결제↔주문, 리스팅↔상품 등 **선언된 참조는 하나도 깨져 있지 않다.**

## 16. FK가 없는 tenant 축 41개 — 고아 4행뿐

문제가 숨을 수 있는 곳은 **FK 제약이 없는 연결 컬럼**이다. `tenant_id` 계열 컬럼 중 FK가 걸리지 않은 것이 **42개**다(`_etl_*` 제외 41개 검사).

```
tenant/행위자 계열 컬럼 123개  →  FK 있음 58 · FK 없음 65
그중 tenant_id 계열 FK 없음 = 42
```

이 41개에 대해 「채워진 값이 `tenants`에 실재하는가」를 검사했다.

| 결과 | |
|---|---|
| 검사한 컬럼 | **41** |
| 존재하지 않는 tenant를 가리키는 컬럼 | **1** |

| 컬럼 | 채워짐 | 고아 |
|---|---|---|
| `admin_logs.tenant_id` | 255 | **4** |

`admin_logs`는 append-only 감사 로그이므로, **삭제된 tenant의 과거 기록이 남은 것**으로 보인다. `ARCH-01` 전제 4(과거 데이터 불변)에 비추면 오히려 정상 동작이다. 실질 피해 없음.

### FK 없는 tenant 컬럼 중 값이 많이 채워진 것 (참고)
```
admin_logs.admin_tenant_id           255      customer_tag_logs.tenant_id        51
admin_logs.tenant_id                 255      customer_tags.tenant_id            51
customer_product_prices.tenant_id    245      commerce_product_listings.*        38
collection_allocations.tenant_id     204      deposit_logs.tenant_id             30
customer_tag_options.tenant_id        67      customer_stats.tenant_id           11
```

## 17. 🔑 이 결과가 바꾸는 해석 — 「끊김」의 성격

§0~§9에서 찾은 단절을 **정확히 다시 규정해야 한다.**

| | |
|---|---|
| ❌ 이런 게 아니다 | "연결이 **깨졌다**" — 잘못된 값, 사라진 부모, 데이터 손상 |
| ✅ 실제는 이렇다 | "연결이 **비어 있다**" — FK 자리가 `NULL`이거나 행 자체가 0 |

| 단절 | 성격 | 실측 |
|---|---|---|
| 리스팅 → 공급자 | **미입력** | `supplier_tenant_id` 채워진 2건은 **전부 유효한 tenant를 가리킨다** |
| 거래처 → 식당 tenant | **미입력** | `linked_tenant_id` 0/142 — 잘못된 값이 아니라 없는 값 |
| 커머스 주문 → 지급원장 | **행 없음** | `supplier_payables` 0행 |
| 발주(RFQ) 전 구간 | **행 없음** | 전부 0행 |
| 식자재 → 메뉴 | **행 없음 + 정책 없음** | `menu_ingredients` 0행 / 정책 미적용(`DR-08`) |

**왜 중요한가**
- **복구 작업이 필요 없다.** 잘못된 데이터를 찾아 고치는 일이 아니라, **빈 칸을 채우는 일**이다.
- `C-18`(리스팅 공급자 지정 36건)·`C-19`(거래처↔tenant 연결 142건)은 **데이터 정합성 문제가 아니라 운영·입력 문제**다.
- 6단계 `K-01`·`K-02`의 난이도 판단이 이 사실 위에서 유지된다 — 선행 작업이 "정합성 복구"였다면 훨씬 비쌌을 것이다.

## 18. 다만 — 무결성이 깨끗한 것은 **지금 규모에서만** 검증된 것이다

| 한계 | |
|---|---|
| 데이터 규모 | 최대 522행(`order_lines`). 소규모라 무결성이 유지되기 쉽다 |
| FK 없는 컬럼 65개 | tenant 축 41개만 검사했다. `*_by`(행위자) 계열은 미검사 |
| 복합 키 FK | 단일 컬럼 FK 144개만 검사했다 |
| 쓰기 경로 | 무결성을 **깨뜨릴 수 있는 경로**(가드 없는 `SECURITY DEFINER` 5개, §12)는 여전히 열려 있다 |

특히 마지막이 중요하다. **지금 무결성이 온전한 것은 그 경로로 아무도 쓰지 않았기 때문일 수 있다.** `create_payment_atomic`처럼 tenant 검증 없이 INSERT하는 함수는 **무결성을 깨는 쪽**이지 지키는 쪽이 아니다.
